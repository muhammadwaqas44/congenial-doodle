...
{
    public function scheduleListing($ComId, Request $request)
    {
        if ($request->ajax()) {
            $data = Schedule::with(['brand', 'subbrand', 'company', 'consultant'])->where('company_id', $ComId)->orderBy('id', 'DESC')->get();
        ...
        return view('dashboard.summary.schedules.schedule-listing', compact('ComId'));
    }
    ...
    public function reviewEvaluation($scheduleId)
    {
        ...
        if ($schedule) {
            ...
            return view('dashboard.summary.details.detail-visit', compact('schedule', 'oultets', 'lodgings', 'marks'));
        } else {
            return abort(404);
        }
    }
    ...
    public function questionMarks($scheduleId, $catId)
    {
        ...
        if ($category) {
            ...
            if ($questionsA || $questionsB) {
                return view('dashboard.summary.question.details', compact(
                    'category',
                    'questionsA',
                    'questionsB',
                    'scheduleId',
    ...
    public function questionOutletMarks($scheduleId, $catId)
    {
        ...
        if ($categories) {
            ...
            if ($questionsA || $questionsB) {
                return view('dashboard.summary.question.outlet-detail', compact(
                    'categoryName',
                    'categories',
                    'questionsA',
                    'questionsB',
    ...
    public
    function summaryReport($scheduleId)
    {
        $schedule = Schedule::find($scheduleId);
        $previousSchedule = Schedule::where('hotel_location_id', $schedule->hotel_location_id)->whereDate('date', '<=', $schedule->date)->orderBy('date', 'desc')->skip(1)->take(1)->first();
        if ($schedule) {
            ...
            return view('dashboard.summary.summary.score', compact(
                'schedule',
                'total_possible',
                'oultets',
                'lodgings',
