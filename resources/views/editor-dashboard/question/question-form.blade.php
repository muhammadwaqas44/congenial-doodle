                                    ...
                                    <div class="card-body card-body-giv-wdth">
                                        <form id="question_marks">
                                            ...
                                            <div style="text-align: center;">
                                                <button type="submit" class="button-graph">Submit and Return Back
                                                </button>
                                            </div>
                                        </form>
                                    </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" id="possible_point" min="0"  onkeypress="return isNumberKey(event)"  onKeyDown="if(this.value.length==2 && event.keyCode!=8) return false;" name="possible_point" type="number" required>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#add_question').on('click', function (e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{route('add-question-schedule-post-editor')}}",
                    data: form,
            ...
            $('form#question_marks').on('submit', function (e) {
                e.preventDefault();
                var form = $('form#question_marks').serialize();
                $.blockUI({
                    css: {
                        border: 'none',
                        padding: '15px',
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{route('marks-value-post-editor')}}",
                    data: form,
                    success: function (response, status) {
                        if (response.result == 'success') {
                            $.unblockUI();
                            window.toastr.success(response.message);
                            setTimeout(function () {
                                window.location.href ="{{route('edit-detail-category-marks-editor',['scheduleId' => $scheduleId,'catId' =>$category->id])}}";
                            }, 1000);
                        } else if (response.result == 'error') {
                            $.unblockUI();
                            window.toastr.error(response.message);
