@extends('layout.consultant-layout.dashboard')

@section('title')
    Add Narrative
@endsection

@section('content')
    <div class="content-page">

        <!-- Start content -->
        <div class="content">

            <div class="container-fluid">

                <div class="row">
                    <div class="col-xl-12">
                        <div class="breadcrumb-holder">
                            <h1 class="main-title float-left">Dashboard</h1>
                            <ol class="breadcrumb float-right">
                                <li class="breadcrumb-item">Home</li>
                                <li class="breadcrumb-item active">Dashboard</li>
                            </ol>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                </div>
                <!-- end row -->

                <div class="row">

                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
                        <div class="card mb-3">
                            <div class="card-header">
                                <h3> Add Narrative</h3>
                            </div>

                            <div class="card-body">
                                <form id="add-narrative-form">
                                    @csrf
                                    <div class="row">

                                        <input type="hidden" name="schedule_id" value="{{$scheduleId}}">
                                        <div class="col-lg-12 col-xl-12">

                                            <div class="row">
                                                <div class="col-lg-6">
                                                    <div class="form-group">
                                                        <label>Category<span class="asterik">*</span></label>
                                                        <select name="category_id" class="form-control" required>
                                                            <option value="" selected>Select</option>
                                                            @foreach($narratives as $narrative)
                                                                <option value="{{$narrative->id}}">{{$narrative->name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-lg-6">
                                                    <div class="form-group">
                                                        <label>Send summary<span class="asterik">*</span></label>
                                                        <select name="send_summary" class="form-control" required>
                                                            <option value="" selected>Select</option>
                                                            <option value="Yes">Yes</option>
                                                            <option value="No">No</option>

                                                        </select>

                                                    </div>
                                                </div>
                                            </div>

                                            <div class="row">
                                                <div class="col-lg-12">
                                                    <div class="form-group">
                                                        <label> Note<span class="asterik">*</span></label>
                                                        <textarea class="form-control" name="notes" id="your_textarea"
                                                                  cols="60"
                                                                  rows="6"></textarea>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="row">
                                                <div class="col-lg-12">
                                                    <button type="submit" class="btn btn-primary btn-chng1">Add
                                                        Narrative
                                                    </button>

                                                </div>
                                            </div>

                                        </div>


                                    </div>

                                </form>

                            </div>
                            <!-- end card-body -->

                        </div>
                        <!-- end card -->

                    </div>
                    <!-- end col -->

                </div>
            </div>
            <!-- END container-fluid -->

        </div>
        <!-- END content -->

    </div>
@endsection




@section('script')
{{--    <script>--}}
{{--        $("#your_textarea").on('keyup', function (event) {--}}
{{--            var count = $("#your_textarea").val().length;--}}
{{--            if (count > 2000) {--}}
{{--                window.toastr.warning('For Note You Add Only 2000 Characters And Your Characters are ' + count + '');--}}
{{--            }--}}
{{--        });--}}

{{--    </script>--}}
    <script>
        $('form#add-narrative-form').on('submit', function (e) {
            e.preventDefault();
            var formData = new FormData(this);
            $.blockUI({
                css: {
                    border: 'none',
                    padding: '15px',
                    backgroundColor: '#000',
                    '-webkit-border-radius': '10px',
                    '-moz-border-radius': '10px',
                    opacity: .5,
                    color: '#fff'
                }
            });
            $.ajax({
                type: 'POST',
                url: "{{route('narrative-post')}}",
                data: formData,
                processData: false,
                contentType: false,
                cache: false,
                success: function (response, status) {
                    if (response.result == 'success') {
                        $.unblockUI();
                        window.toastr.success(response.message);
                        setTimeout(function () {
                            window.location.href = '{{route('visit-detail-consultant',$scheduleId)}}';
                        }, 1000);
                    } else if (response.result == 'error') {
                        $.unblockUI();
                        window.toastr.error(response.message);
                    }
                },
                error: function (data) {
                    $.each(data.responseJSON.errors, function (key, value) {
                        $.unblockUI();
                        window.toastr.error(value);
                    });
                }
            });
        });
    </script>

@endsection
